...
if [ $IP1 ]; then
	LOCAL="%F{white}─🮤 %F{cyan}$IP1%F{white}🮥"
else
	LOCAL=""
fi

if [ $IP2 ]; then
    VPN="%F{white}─🮤 %F{green}$IP2%F{white}🮥"
else
    VPN=""
fi

if [ $IP3 ]; then
    WIFI="%F{white}─🮤直 %F{green}$IP3%F{white}🮥"
else
    WIFI=""
fi
...
if [ "$color_prompt" = yes ]; then
    ...
    RPROMPT=$'%F{white}[%F{reset}'$TIME'%(?.. %? %F{white}%B⨯%b%F{reset})%(1j. %j %F{yellow}%B⚙%b%F{reset}.)%F{white} ]'
	...
	ZSH_HIGHLIGHT_STYLES[commandseparator]=fg=orange,bold
	ZSH_HIGHLIGHT_STYLES[autodirectory]=fg=white,underline
	ZSH_HIGHLIGHT_STYLES[path]=fg=yellow,underline
	ZSH_HIGHLIGHT_STYLES[path_pathseparator]=
	ZSH_HIGHLIGHT_STYLES[path_prefix_pathseparator]=
	ZSH_HIGHLIGHT_STYLES[globbing]=fg=orange,bold
	ZSH_HIGHLIGHT_STYLES[history-expansion]=fg=orange,bold
	...
	ZSH_HIGHLIGHT_STYLES[back-quoted-argument-delimiter]=fg=orange,bold
	...
	ZSH_HIGHLIGHT_STYLES[bracket-level-1]=fg=orange,bold
